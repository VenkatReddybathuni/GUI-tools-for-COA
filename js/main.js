...
function updatePipelineVisualization(stage, details) {
    const stageElements = {
        'machine-code': document.getElementById("stage-machine-code"),
        'fetch': document.getElementById("stage-fetch"),
        ...
        'writeback': document.getElementById("stage-writeback"),
    };

    if (stageElements[stage]) {
        stageElements[stage].innerHTML = details;
        stageElements[stage].classList.add('bg-success', 'text-white');
        setTimeout(() => {
            stageElements[stage].classList.remove('bg-success', 'text-white');
        }, 1000);
    }
}
$(document).ready(function() {
    ...
    var me = mipsEmulator({
        ...
        onFinish: function() {
            ...
            me.setLine(1);
            setHighlights({
                lineRan: lastLineNoRun,
                nextLine: me.getLineNumber()
            });
            running = false;
        },
        /*
    ...
    var nextMarker = editor.markText({
        line: me.getLineNumber() - 1,
        ch: 0
    }, {
        line: me.getLineNumber(),
        ch: 0
    }, {
        title: "Next line to be run",
        className: 'nextLine'
    });
    ...
    $("#autoSwitch").change(function(e) {
        autoSwitch = $(e.target).is(':checked');
    });
    ...
    // In main.js, near other utility functions
