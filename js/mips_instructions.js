function mipsInstructionExecutor(ME) {
    ...
    var instructions = {
        ...
        /////////////////////////////////////////////
        'MOVE': {
            parseMethod: parse_$RD_$rs,  // Using parse_$RD_$rs to parse the destination and source registers
            runMethod: function(namedArgs) {
                ME.setRegisterVal(namedArgs.$rd, ME.getRegisterVal(namedArgs.$rs));
                ME.incrementPC();
            }
        },
        'SYSCALL': {
    ...
    var result = {
        parseInstruction: function(instructionName, args, outArgs, outError) {
            ...
            var validArgs = true;
            var namedArgs = parseMethod(args); // Parse arguments

            if (namedArgs['expectedArgCount'] !== args.length) {
                if (outError) outError.message = "Incorrect number of arguments " + args.length + ", should be " + namedArgs['expectedArgCount'];
                return false;
            }
